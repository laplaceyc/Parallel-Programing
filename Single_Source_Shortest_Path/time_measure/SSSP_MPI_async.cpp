	...
	if(rank == source){
		for(int i = 0; i < vertex_num; i++){
			...
			//	int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm)
				double comm_st = MPI_Wtime();
				MPI_Send(&new_distance, 1, MPI_INT, i, BELLMAN_TAG, MPI_COMM_WORLD);
				double comm_ed = MPI_Wtime();
				comm += (comm_ed - comm_st);
			...
			if(recv_distance < distance){
				...
				for(int i = 0; i < vertex_num; i++){
					...
					//if there is a path
						...
						double comp_st = MPI_Wtime();
						if(i < rank) token = black;
						int new_distance = distance + local_adj_matrix[i];
						double comp_ed = MPI_Wtime();
...
//			printf("\n");
	...
	if(rank == 0) {
		printf("[exec][%lf]\n", ed - st);
		printf("[rank][comm][comp][comm_g][comm_t][i/o][exec][msg]\n");
	}
	printf("%d %lf %lf %lf %lf %lf %lf %d\n", rank, comm, comp, (comm_g_ed - comm_g_st), comm + (comm_g_ed - comm_g_st), (io_ed1-io_st1)+(io_ed2-io_st2), ed - st, msg);
	MPI_Finalize();
