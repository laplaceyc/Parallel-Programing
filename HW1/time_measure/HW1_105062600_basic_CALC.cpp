...
int main (int argc, char *argv[]) {
	...
	// Two judge method
	/*
	if ((N % size) && (rank < (N % size))) {//allocate remainder N
		 num_per_rank++;
	}
	*/
	offset = rank * num_per_rank * sizeof(int);
	int head = rank * num_per_rank;
	...
	*/
	...
	while (!all_sorted) {
	double commu_start_time_2 , commu_end_time_2 ;
	double commu_start_time_1 , commu_end_time_1 ;
		sorted = true;
		int i;
	...
	commu_start_time_1 = MPI_Wtime();
	if(rank != 0 && head % 2 == 0){
		sorted = HEADCOMPARE(rank, local_buf, custom_world, sorted);
		}
	commu_end_time_1 = MPI_Wtime();
	...
	commu_start_time_2 = MPI_Wtime();
	if(rank != 0 && head % 2 == 1){
		sorted = HEADCOMPARE(rank, local_buf, custom_world, sorted);
		}
	commu_end_time_2 = MPI_Wtime();
