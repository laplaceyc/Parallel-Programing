...
int main (int argc, char *argv[]) {
	...
	if(size > 1){
		...
		if(rank == 0){
			...
			//int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm)
			MPI_Send(local_buf, POINT_NUM_Y, MPI_INT, 0, BEGIN_TAG, custom_world);
			int task_column;
			//int MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status *status)
			while(MPI_Recv(&task_column, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status) == MPI_SUCCESS) {
				if(status.MPI_TAG == TERMINATION_TAG) break;
				local_buf[0] = task_column;
				#pragma omp parallel num_threads(THREAD_NUM) private(i, c)
				{	
